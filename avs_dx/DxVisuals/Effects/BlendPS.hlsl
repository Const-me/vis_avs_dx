...
{
    const int3 pos = int3( int2( screenSpace.xy ), 0 );
    const float4 s = texSource.Load( pos );
    const float4 d = texSource.Load( pos );

	// `blend` will become macro in runtime, i.e. these branches are free, and hopefully, HLSL compiler will even optimize away unused texture loads.
    switch( blend )
    {
        case 0: // Ignore
            return d;
